...
- **目录卡片封面**: 对于 `posts/` 根目录以外的目录,如果该目录下存在 `cover.jpg`、`cover.png` 或 `cover.webp`,目录卡片会使用它作为封面; 如果不存在,该目录卡片不显示封面
...
- 这里的 `cover.*` 属于目录卡片资源,不依赖某篇文章 frontmatter 里的 `image`
- 只检查当前目录,不会递归向子文章或子目录借用图片
- 支持的文件名当前固定为 `cover.jpg`、`cover.png`、`cover.webp`
