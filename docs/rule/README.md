...
**说明**：规范当前统一侧栏渲染入口下的组件接入流程，避免"配置了组件但页面不显示"的遗漏。

**关键点**：
- 侧栏组件接入的 4 个必要步骤
- 在 `WidgetComponentType` 中声明组件类型
- 在 `sidebarLayoutConfig` 中配置布局
- **在 `SidebarColumn.astro` 的 componentMap 中注册组件**（当前唯一统一入口）
- 在 `buildComponentProps` 中集中处理特殊 props
- `toc` / `card-toc` 目录组件的当前接口
- 常见问题排查

### 7. [当前技术接口速查](../CURRENT_INTERFACES.md)

**说明**：记录本仓库相对上游 Mizuki 的当前接口。

**关键点**：
- TOC / 目录树的 Sidebar、Card、Mobile、Floating 入口
- 侧栏 widget 接入链路
- Shiki / Expressive Code 高亮配置
- Markdown 处理链和自定义语法
...
- [ ] 暗色主题使用 CSS 变量实现
- [ ] **侧栏组件已在 `SidebarColumn.astro` 的 componentMap 中注册**
- [ ] **侧栏组件的类型已在 `WidgetComponentType` 中声明**
- [ ] **侧栏组件已在 `sidebarLayoutConfig.components` 中配置**
- [ ] **目录树 / 代码高亮 / Markdown 接口变化已同步 `CURRENT_INTERFACES.md`**
