...
│   │   │   ├── protection/       # 密码保护
│   │   │   ├── media/            # 媒体相关
│   │   │   └── toc/              # TOC 核心入口、hooks、utils、types
│   │   │
│   │   ├── layout/           # 布局片段组件
│   │   │   ├── SidebarColumn.astro
│   │   │   └── RightSideBar.astro
...
├── ec.config.mjs                # Expressive Code / Shiki 配置
...
> 当前 TOC 不放在 `organisms/TOC/`。目录核心入口位于 `src/components/features/toc/`，卡片式侧栏目录位于 `src/components/widgets/card-toc/`，兼容包装器位于 `src/components/widgets/toc/`。
...
- `widget-renderer.ts` - Widget props 组装
- `tocManager.ts` - CardTOC 客户端目录管理器
