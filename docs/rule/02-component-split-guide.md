...
**背景**：
- 当前 TOC 有桌面侧栏、卡片式侧栏、移动端面板、悬浮面板 4 个入口。
- 它们共用“标题提取、深度过滤、徽章生成、滚动定位”这类逻辑。
- UI 容器和运行时生命周期不同，不能强行合并成一个大组件。

**拆分策略**：
1. 将共享算法放在 `src/components/features/toc/utils/`。
2. 将滚动、高亮、导航等副作用放在 `src/components/features/toc/hooks/`。
3. 将不同入口分别保留为 `SidebarTOC`、`MobileTOC`、`FloatingTOC`。
4. 将卡片式侧栏目录作为 widget 放在 `src/components/widgets/card-toc/`，复用 `src/utils/tocManager.ts`。

**拆分结果**：
```
src/components/features/toc/
├── SidebarTOC.astro
├── MobileTOC.svelte
├── FloatingTOC.astro
├── components/
...
src/components/widgets/card-toc/
└── CardTOC.astro
