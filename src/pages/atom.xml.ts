...
const svgImagesGlob = import.meta.glob<string>("/src/content/**/*.svg", {
	import: "default",
	query: "?url",
});
		...
		for (const img of images) {
			...
			if (
				...
				if (imageMod) {
					...
					const svgUrl = await svgImagesGlob[importPath]?.();
					if (svgUrl) {
						img.setAttribute("src", new URL(svgUrl, context.site).href);
					} else {
